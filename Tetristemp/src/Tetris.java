...
public class Tetris extends JFrame implements Runnable, KeyListener {
	...
	public Tetris(String str) {
		...
		for (int i = 0; i < background.length; i++) {
			for (int p = 0; p < background[i].length; p++) {
				...
				this.background[i][p].setBounds(i * this.area, p * this.area,
						this.area, this.area);
		...
		this.currentItem.setColor(this.colorList.get(this.rnd
				.nextInt(this.colorList.size())));
	...
	public void setNextItem() {
		...
		this.nextItem = temp;
		this.nextItem.setColor(this.colorList.get(this.rnd
				.nextInt(this.colorList.size())));
	...
	public void setCopyBlock() {
		...
		for (int i = 0; i < tempBlock.length; i++) {
			setBack(tempBlock[i].getX(), tempBlock[i].getY(), this.currentItem
					.getColor());
		}
	...
	public void goRotate() {
		...
		for (int i = 0; i < tempBlock.length; i++) {
			...
			if (x < 0 || x >= this.xCnt || y + 1 >= this.yCnt
					|| this.grid[x][y]) {
				return;
			}
		}
	...
	// 게임종료체크
	public void gameEnd() {
		JOptionPane.showMessageDialog(null, "게임이 종료되었습니다.", "게임종료",
